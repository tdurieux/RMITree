...
for i in {2..20}
do
    printf "[$i] did receive message \"testMessage1\" from [1] ? ";
    grep "\[$i\] \"testMessage1\" receives message from 1" out.txt | wc -l | awk \
            'BEGIN {} $1 == "1" { print "Yes" } $1 != "1" { print "No " $1 }'
    printf "[$i] did receive message \"testMessage2\" from [1] ? ";
    grep "\[$i\] \"testMessage2\" receives message from 1" out.txt | wc -l | awk \
            'BEGIN { } $1 == "1" { print "Yes" } $1 != "1" { print "No " $1 }'
done
